...
from korail_bot.macro import KST, MacroManager, SeatPref, pick_seat_class
...
class FakeKorail:
    ...
    async def reserve(self, t, seat_class, adults):
        if self.reserve_errors:
            raise self.reserve_errors.pop(0)
        self.reserved.append((t.train_no, seat_class))
...
def manager(fake, results):
    async def on_success(job, hold, t, seat_class):
        results.append(("ok", t.train_no, seat_class))
